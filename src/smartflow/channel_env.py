...
class ChannelEnv(CFDEnv):
    ...
    def __init__(self, conf, runtime):
        ...
        for i in range(self.n_cases):
            ...
            case_path = os.path.join(self.cwd, "cases", conf.environment.case_names[i])
    ...
    def _recalculate_reward(self):
        ...
        agents_per_cfd = self.agents_per_cfd
        for i in range(self.n_cfds):
            reward = self._cfd_rewards[i, :].reshape(agents_per_cfd, self.cfd_reward_dim)
    ...
    def _recalculate_action(self):
        ...
        for i in range(self.n_cfds):
            for j in range(agents_per_cfd):
                for k in range(self.cfd_action_dim):   # action is a single value
                    self._cfd_actions[i, j * self.cfd_action_dim + k] = scaled_actions[i * agents_per_cfd + j, k]